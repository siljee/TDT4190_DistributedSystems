...
public class TicTacToe extends JFrame implements ListSelectionListener
{
  ...
  {
	  // Wait for user to determine if object is client or server.
  ...
  public TicTacToe(boolean server)
  {
    ...
    if (server) {
		...
		try {
			// Makes a reference of itself available to the client at the given location.
	    ...
	    try {
	    	// Get the reference of the opponents object and send a reference of itself.
  ...
  /** 
   * Implements interface made in MoveServer.java. These methods can be invoked remotly.  
   */
  public class MoveServerImpl extends UnicastRemoteObject implements MoveServer{
	  ...
	  /**
	   *  Sends a reference of this object to the opponent.
	   */
	  @Override
	  ...
	  /**
	   *  Upates borad when opponent have invoked this method remotly.
	   */
	  @Override
